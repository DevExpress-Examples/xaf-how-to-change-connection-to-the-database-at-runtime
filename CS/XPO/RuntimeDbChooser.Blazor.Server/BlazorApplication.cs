...
namespace RuntimeDbChooser.Blazor.Server {
    public class RuntimeDbChooserBlazorApplication : BlazorApplication {
        ...
        protected override bool IsCompatibilityChecked {
            get => isCompatibilityChecked.ContainsKey(ServiceProvider.GetRequiredService<ConnectionStringProvider>().GetConnectionString());

            set => isCompatibilityChecked.TryAdd(ServiceProvider.GetRequiredService<ConnectionStringProvider>().GetConnectionString(), value);
        }
